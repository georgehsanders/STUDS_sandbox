...
@app.route('/hq/_debug/sku-eligibility/<sku>')
@hq_login_required
def hq_debug_sku_eligibility(sku):
    """Temporary debug route — diagnose why a SKU may be excluded from the assignment algorithm."""
    sku = sku.upper().strip()
    from datetime import datetime, timedelta

    out = {"sku": sku, "checks": {}}

    # 1. Master file presence
    master = load_master_skus()
    out["checks"]["in_master"] = sku in master
    out["checks"]["master_description"] = master.get(sku)
    out["checks"]["master_total_skus"] = len(master)

    # 2. Status file presence + value
    status_map = load_sku_status()
    out["checks"]["status_value"] = status_map.get(sku, "(no entry — defaults to active)")
    out["checks"]["status_total_entries"] = len(status_map)

    # 3. Top sellers file presence + rank
    top = load_top_sellers()
    rank = None
    for row in top:
        if row.get("sku", "").upper() == sku:
            rank = row.get("rank")
            break
    out["checks"]["top_sellers_rank"] = rank
    out["checks"]["top_sellers_total"] = len(top)

    # 4. first_seen_at value from sku_first_seen table
    conn = sqlite3.connect(STORE_DB)
    conn.row_factory = sqlite3.Row
    try:
        cursor = conn.execute("SELECT first_seen_at FROM sku_first_seen WHERE sku = ?", (sku,))
        row = cursor.fetchone()
        if row:
            out["checks"]["first_seen_at"] = row[0]
            try:
                fs_str = row[0]
                fs = datetime.fromisoformat(fs_str.replace(" ", "T")) if isinstance(fs_str, str) else fs_str
                days_since = (datetime.now() - fs).days
                out["checks"]["days_since_first_seen"] = days_since
                out["checks"]["passes_new_sku_delay"] = days_since >= 28
            except Exception as e:
                out["checks"]["first_seen_parse_error"] = str(e)
        else:
            out["checks"]["first_seen_at"] = None
            out["checks"]["passes_new_sku_delay"] = False
    finally:
        conn.close()

    # 5. Last counted across the network (for context)
    conn = sqlite3.connect(STORE_DB)
    conn.row_factory = sqlite3.Row
    try:
        cursor = conn.execute(
            """SELECT MAX(sc.completed_at) as last_counted
               FROM stock_check_skus scs
               JOIN stock_checks sc ON scs.stock_check_id = sc.id
               WHERE scs.sku = ? AND sc.status = 'completed' AND sc.completed_at IS NOT NULL""",
            (sku,)
        )
        row = cursor.fetchone()
        out["checks"]["last_counted_at"] = row["last_counted"] if row and row["last_counted"] else "never"
    finally:
        conn.close()

    # 6. Verdict
    eligible = (
        out["checks"]["in_master"]
        and out["checks"]["status_value"] != "sunset"
        and out["checks"].get("passes_new_sku_delay", False)
    )
    out["eligible"] = eligible

    if not eligible:
        reasons = []
        if not out["checks"]["in_master"]:
            reasons.append("not in master")
        if out["checks"]["status_value"] == "sunset":
            reasons.append("status is sunset")
        if not out["checks"].get("passes_new_sku_delay", False):
            reasons.append(
                f"failed new-SKU delay (days_since_first_seen={out['checks'].get('days_since_first_seen')})"
            )
        out["exclusion_reason"] = " AND ".join(reasons)

    # 7. Path diagnostics
    out["paths"] = {
        "MASTER_DIR": MASTER_DIR,
        "Top_Sellers.csv exists": os.path.isfile(os.path.join(MASTER_DIR, "Top_Sellers.csv")),
        "SKU_Master.csv exists": os.path.isfile(os.path.join(MASTER_DIR, "SKU_Master.csv")),
        "SKU_Status.csv exists": os.path.isfile(os.path.join(MASTER_DIR, "SKU_Status.csv")),
    }
